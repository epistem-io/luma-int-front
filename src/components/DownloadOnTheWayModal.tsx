...
export interface DownloadOnTheWayModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onConfirm?: () => void;
}

export default function DownloadOnTheWayModal({
  open,
  onOpenChange,
  onConfirm,
}: DownloadOnTheWayModalProps) {
  const t = useTranslations("DownloadOnTheWayModal");
      ...
      >
        <div className="flex flex-col items-center gap-4 text-center">
          <div className="flex size-[52px] items-center justify-center rounded-full border-[3px] border-primary-pink text-primary-pink">
            <Mail className="size-7 stroke-[2.5]" aria-hidden="true" />
          </div>
